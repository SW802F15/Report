...
/*
     FILE ARCHIVED ON 20:16:40 mar 16, 2007 AND RETRIEVED FROM THE
...
<!-- 2006/10/30: Modified by Anthony Colon -->
...
function dcsVar(){
            ...
            if (document.title){
            		var tempTitle=document.title.replace( /^http.*\s\-\s/);
                        WT.ti=gI18n?dcsEscape(dcsEncode(tempTitle),I18NRE):tempTitle;
            }
...
function dcsDownload(evt){
	...
	if (evt){
		...
		if (e.hostname&&dcsIsOnsite(e.hostname)){
			var types="xls,doc,pdf,txt,csv,zip,mp3,ppt,mpp";
			if (types.indexOf(e.pathname.substring(e.pathname.lastIndexOf(".")+1,e.pathname.length)) > 0){
				...
				if (e.pathname.substring(e.pathname.lastIndexOf(".")+1,e.pathname.length)=="pdf"){
					dcsMultiTrack("DCS.dcssip",e.hostname,"DCS.dcsuri",path,"DCS.dcsqry",e.search||"","WT.ti","Download:"+(e.innerHTML||""),"WT.dl","1","DCSext.DocAction","PDF");
				} 
				else {
		...
		if (btn!=1){
			...
			if (dcsIsHttp(e)){
				...
				if (types.indexOf(e.pathname.substring(e.pathname.lastIndexOf(".")+1,e.pathname.length)) > 0){
					var path=e.pathname?((e.pathname.indexOf("/")!=0)?"/"+e.pathname:e.pathname):"/";
					if (e.pathname.substring(e.pathname.lastIndexOf(".")+1,e.pathname.length)=="pdf"){
						dcsMultiTrack("DCS.dcssip",e.hostname,"DCS.dcsuri",path,"DCS.dcsqry","","WT.ti","Download:"+path,"WT.dl","1","WT.rc","1","DCSext.DocAction","PDF");
					} 
					else {
...
function getUserCookieVal (offset) {
...
var wtusercookie=GetUserCookie("UserType");
if (wtusercookie != null) {
	WT.seg_2=wtusercookie;
}else{
	WT.seg_2="000000-00";
}
