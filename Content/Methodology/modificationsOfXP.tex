...
The purpose of this practise is to ensure energetic developers every day, and not to overwork them to the point of burnout \citet[p. 58]{xp:explored}.
...
This practise supports flexible and quick changes \citet[p. 54]{xp:explored}, however some risks still exist.
...
According to XP this practise should be automated through the setup of a dedicated build server\citet[p. 57]{xp:explored}.
...
If the tests are not at a 100\%, the developers must fix their code or discard it, to ensure the build always passes 100\% of the tests. %bahahaha
...
A meaningful metaphor helps create and name the correct objects and actions \citet[p. 87]{xp:explored}.
...
Planning is important. It lets you know how fast you work, what to do next, provides an effective means of communication with the customer, and allows you to quickly respond to changes \citet[p. todo]{xp:explored}. 
\Kristian{grep for bad cites?}

In XP planning happens in two stages. There is the release planning and the iteration planning.
The release planning event is where the customer writes all the user stories, he wants the program to cover\citet[p. todo]{xp:explored}.
...
The iteration planning event is where the customer selects which of the stories, from the release plan, he wants implemented in the upcoming iteration \citet[p. todo]{xp:explored}.
...
XP prescribes refactoring to be done on the spot when a bad smell is identified, to prevent building on top of bad code \citet[p. todo]{xp:explored}.
...
The purpose of this practise is to reduce the risks if changes in the use domain occur\citet[p. todo]{xp:explored}.
