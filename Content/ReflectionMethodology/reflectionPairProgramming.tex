...
One of the major concerns about pair programming is if it is productive enough to ``seal'' the work force of two developers for a single story.
We are not in a position to conclude this, but in our opinion it depends on the team using it.
...
This would usually lead to both developers reading the same material and experimenting with the same solutions.
This can be seen as unproductive, since two developers are during the same task.
...

Secondly we discovered that if part of the pair got distracted, the other would likely get sucked in as well.
This gave rise to some situations where both developers unknowingly supported each others slacking.
